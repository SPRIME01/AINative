    ...
    "backend": {
        "src": {
            "ainative": {
                ...
                "config": [".gitkeep"], # litellm.config.yaml is generated by another script
                ...
                "prompts": { # New prompts directory for agent-specific prompts
                    ...
                    "strategist": ["system_prompt.md", "user_examples.md", "README.md", ".gitkeep"],
                    "research": ["system_prompt.md", "user_examples.md", "README.md", ".gitkeep"],
                    "builder": ["system_prompt.md", "user_examples.md", "README.md", ".gitkeep"],
                    "planner": ["system_prompt.md", "user_examples.md", "README.md", ".gitkeep"],
                    "librarian": ["system_prompt.md", "user_examples.md", "README.md", ".gitkeep"],
                    "scribe": ["system_prompt.md", "user_examples.md", "README.md", ".gitkeep"],
                    "critic": ["system_prompt.md", "user_examples.md", "README.md", ".gitkeep"],
                    "guardian": ["system_prompt.md", "user_examples.md", "README.md", ".gitkeep"]
                }
            }
        },
        "tests": {
...
"""
    ...
    "frontend": { # Basic structure, assuming it exists or can be created
        "public": [".gitkeep"], "src": [".gitkeep"], "index.html": "<!-- Frontend entry point -->", "package.json": "{ \"name\": \"ainative-frontend\", \"version\": \"0.1.0\" }"
    },
...
def safe_create_structure(base_path, structure_definition, root_call=True):
    """
    ...
    for name, content in structure_definition.items():
        ...
        if name == ".gitkeep" and current_item_path.parent.exists() and any(current_item_path.parent.iterdir()):
        ...
        if isinstance(content, dict):  # It's a directory
            ...
            safe_create_structure(current_item_path, content, root_call=False)  # Recurse
            ...
            for item_in_list in content:
                if isinstance(item_in_list, str): # File name
                    ...
                    for sub_dir_name, sub_dir_content in item_in_list.items():
                        ...
                        safe_create_structure(current_item_path, {sub_dir_name: sub_dir_content}, root_call=False)

        elif isinstance(content, str):  # It's a file, content is the initial string for the file
            write_content = False
            current_content = ""
            ...
            elif name in ["pyproject.toml", ".gitignore", "pre-commit-config.yaml", "Dockerfile", "README.md", "requirements.txt"] and content.strip():
                 if not current_content.strip() or "Auto-generated" in current_content or "# Project Edge AI" in current_content or "[tool.poetry]" in current_content : # if it's empty or looks like a template
                    write_content = True
                 else:
                    print(f"File exists with user content, skipped update: {current_item_path}")
            elif not current_content.strip() and content.strip(): # File exists but is empty, and new content is not empty
                write_content = True
...
if __name__ == "__main__":
    ...
    if PROJECT_ROOT.name != "project_edge_ai":
        print(f"Warning: The script is expected to be in 'project_edge_ai/scripts/'.")
        print(f"Current project root is '{PROJECT_ROOT.name}'. Please ensure this is correct.")
    ...
    print("You may need to run 'poetry install' in 'project_edge_ai/backend/' if pyproject.toml was updated/created.")
    print("Ensure 'otel-collector-config.yaml' and 'litellm.config.yaml' are correctly placed or generated in 'project_edge_ai/backend/src/ainative/config/'.")
