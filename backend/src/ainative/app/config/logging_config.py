...
def setup_loguru_for_test(
    ...
    file_handler_id = loguru_logger.add(
        ...
        level="INFO",
        format="{message}", # We rely on serialize=True and record dict structure
        enqueue=True, # Important for async or multiprocessing
        encoding="utf-8" # Specify UTF-8 encoding for the file sink
    )
    ...
    otel_handler_id = loguru_logger.add(
        otel_sink,
        level="INFO",
        format="{message}", # Format is less relevant here as we use the record directly
        enqueue=True
    )
...
@app.get("/execute-log-test")
async def execute_log_test_endpoint(request: Request, x_correlation_id: str | None = None):
    """
...
def test_loguru_fastapi_integration(
    ...
    expected_path = "/execute-log-test"
    ...
    assert "timestamp" in log_entry["time"], "Timestamp missing in file log."
